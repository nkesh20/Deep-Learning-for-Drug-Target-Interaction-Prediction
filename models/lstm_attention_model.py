...
def build_lstm_attention_model(config):
    # Inputs
    ...
    drug_lstm = LSTM(config['lstm_units'], return_sequences=True)(drug_embedding)
    ...
    protein_lstm = LSTM(config['lstm_units'], return_sequences=True)(protein_embedding)

    # Attention
    attention = Attention()([drug_lstm, protein_lstm])

    # Concatenate
    concat = Concatenate()([tf.keras.layers.GlobalAveragePooling1D()(attention),
                            tf.keras.layers.GlobalAveragePooling1D()(protein_lstm)])
